...
void Example1::createViewports()
{
	 ...
	 mCamTop->setAspectRatio(Real(vp2->getActualWidth()) / Real(vp2->getActualHeight()));
	 mCamLeft->setAspectRatio(Real(vp3->getActualWidth()) / Real(vp3->getActualHeight()));
	 mCamFront->setAspectRatio(Real(vp4->getActualWidth()) / Real(vp4->getActualHeight()));
}

void Example1::createFrameListener()
{
	mFrameListener = new Example1FrameListener(mSceneMgr,mWindow,mCamera);
